...
const UpdateUserRoleModal = ({ isOpen, closeModal, role, refetch, data }) => {
  const [updatedRole, setUpdatedRole] = useState(data?.role);
  //* function handleApproved
  const axiosSecure = useAxiosSecure();
  const handleApproved = async () => {
    try {
      await axiosSecure.patch(`/updateRole/${data?._id}`, {
        email: data.email,
        role: updatedRole,
